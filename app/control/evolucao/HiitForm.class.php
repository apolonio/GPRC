<?php
/**
 * @class:     HiitForm= Responsável pela edição do formulário
 * @version    1.0
 * @package    icriacoes
 * @subpackage gprc
 ...
 */
class HiitForm extends TStandardForm
{
    protected $form;
    ...
    {
        parent::__construct();
        $this->form = new TQuickForm('form_hiit');
        $this->form->class = 'tform'; 
        $this->form->style = 'width: 700px;';
        $this->form->setFormTitle('HIIT' );
        parent::setDatabase('permission'); 
        parent::setActiveRecord('AvaliacaoHiit');
        
        $codHiit = new TEntry('codHiit');
        ...
        $lbl_fcfinal->setSize(60);
         ...
         $codHiit->setEditable( FALSE );
         
        // Campos que aparecem no Formulário
        $this->form->addQuickField('ID',$codHiit,100);
        $this->form->addQuickField('Paciente',$codPaciente,350);
        $this->form->addQuickField('Avaliador',$codAvaliador,350);   
        $this->form->addQuickField('Sessão',$sessao,100);   
        ...
        $this->form->addQuickAction(_t('Find'), new TAction(array('PesquisaHiit', 'onReload')), 'ico_back.png');

        $vbox = new TVBox;
        $vbox->add(new TXMLBreadCrumb('menu.xml', 'PesquisaHiit'));
        $vbox->add($this->form);
